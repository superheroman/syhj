  ...
  <div class="margin-top">
    <el-card class="table-wrap" header="电子料单价录入界面" v-loading="tableLoading">
      <el-table :data="electronicBomList" height="75vh">
        ...
        <el-table-column label="ENCC码" width="150">
          <template #default="{ row }">
            <el-select :disabled="!row.isEdit" v-model="row.ECCNCode" placeholder="选择ENCC码">
              <el-option v-for="item in ENCCOptions" :key="item.label" :label="item.label" :value="item.label" />
            </el-select>
          </template>
        </el-table-column>
...
const submitFun = async (_record: ElectronicDto, isSubmit: number, index: number) => {
