...
const { auditFlowId = 1, modelCountId = 3 }: any = getQuery()
...
const getGoTableChartData = async () => {
  const {
    result: { items = [] }
  }: any = await GetGoTable({ AuditFlowId: auditFlowId, ModelCountId: modelCountId, InputCount: data.productInputs })
